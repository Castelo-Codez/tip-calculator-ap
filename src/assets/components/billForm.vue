...
onMounted(() => {
    ...
    let $billForm = document.querySelector("#tip-input-form");
    $billForm.addEventListener("submit", (e) => {
        e.preventDefault();
    });
    $billForm.addEventListener("input", (e) => {
        if (isNaN($billForm.tip.value)) {
            $error(true);
        } else {
            $error(false);
            let $ob = JSON.parse(window.localStorage.getItem("target-obj"));
            $ob.bill = +$billForm.tip.value;
            window.localStorage.setItem("target-obj", JSON.stringify($ob));
            calc();
        }
    });
    $billForm.tip.addEventListener("blur", (e) => {
        if (
            document.querySelector("#number-of-people").nmbrofpeople.value == ""
        ) {
            $errorMsg[1].classList.add("active");
            $errorMsg[1].textContent = "can't be zero";
            document
    ...
    function $error($status) {
        if ($status) {
            $errorMsg[0].textContent = "input must be a number";
...
<template>
    <form id="tip-input-form">
        <div class="top-layer">
            <label for="tip-input">bill</label>
